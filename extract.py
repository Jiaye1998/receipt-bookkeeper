...
# 每个类别的判断依据。v2 新增:eval 发现模型一拿不准就往"购物/其他"靠,
# 把药房、物业、仓储店等判错。给出通用定义来提升判断力(注意:是通用规则,
# 不是针对测试集里某几张"背答案",否则就是过拟合)。
CATEGORY_GUIDE = """\
- 餐饮:餐厅、快餐、外卖、咖啡奶茶、火锅等
- 超市:超市、便利店、仓储会员店(如沃尔玛、Costco、家乐福、山姆)
- 交通:打车、公交地铁、加油、停车、机票火车票
- 购物:服装、数码电器、日用百货等"非超市"的零售店
- 医疗:药房、药店、医院、诊所、体检
- 娱乐:电影、KTV、游戏、景点门票、健身
- 住房:房租、物业费、水电燃气费、宽带
- 其他:实在归不进以上任何一类时,才用这个"""

# 用哪个模型。gpt-4o-mini 便宜、支持看图和结构化输出,够用。
# 想更准可以换成 "gpt-4o"。改这一行就行。
MODEL = "gpt-4o-mini"

# 懒加载客户端:第一次真正用到时才创建(避免一导入就因没读到 key 而崩)
_client = None
...
def _get_client() -> OpenAI:
    global _client
    if _client is None:
        _client = OpenAI()  # 自动从环境变量 OPENAI_API_KEY 读 key
    return _client
                ...
                "content": [
                    {
                        ...
                        "text": (
                            ...
                            "判断类别时,先看商家是做什么的,再按下面的依据归类;"
                            "只有实在归不进前面任何一类时,才选'其他':\n"
                            f"{CATEGORY_GUIDE}"
                        ),
                    },
                    {
